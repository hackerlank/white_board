...
public:
	...
	uint8*	buffer() { return &_storage[0];};
...
protected:
	...
	size_t _rpos, _wpos;
	std::vector<uint8,SAllocator<uint8> > _storage;
};
...
template <typename T> ByteBuffer &operator<<(ByteBuffer &b, std::vector<T,SAllocator<T> > v)
{
	b << (uint32)v.size();
	for (typename std::vector<T,SAllocator<T> >::iterator i = v.begin(); i != v.end(); ++i) {
		b << *i;
	}
	return b;
}
